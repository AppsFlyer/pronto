...
(def abstract-types-bodies (atom {}))
...
(defn expand-deftype [x]
  (let [abstract-types       (->> x
        ...
        abstract-type-bodies (binding [*expanded-types* (union *expanded-types* abstract-types)]
                               (->> abstract-types
                                    (map #(vector (ns-name (-> % meta :ns))
                                                  (name (-> % meta :name))))
                                    (map (partial @abstract-types-bodies))
                                    (map clean-deftype)
                                    (map expand-deftype)
                                    (map deftype->deftype*)
                                    doall))]
...
(defmacro def-abstract-type
  ...
  [name & body]
  (let [k    [(ns-name *ns*) (clojure.core/name name)]
        form `(deftype ~name [] ~@body)]
    (swap! abstract-types-bodies assoc k form)
    `(def
